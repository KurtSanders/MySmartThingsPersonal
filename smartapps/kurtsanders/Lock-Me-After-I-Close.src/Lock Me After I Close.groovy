...
def doorHandler(evt)
{
    ...
    def cc = contact.currentContact
    def cls = lock.currentState("lock").value
    def ccs = contact.currentState("contact").value
    
    log.debug "Event: Name ${evt.name} is ${evt.value} value and created at: ${evt.date}"
    log.debug "latestContactValue: $cs"
    log.debug "currentContact: $cc"
    log.debug "latestLockValue: $ls"
    log.debug "event description text: ${evt.description}"
    ...
    log.debug "The value of this event as a string: ${evt.stringValue}"
 	log.debug "lock.currentState = $cls"
 	log.debug "contact.currentState = $ccs"
...
// 	Door has Been OPENED, Lock is UNLOCKED (Normal Door Opening Mode)
    else if ((lock.latestValue("lock") == "unlocked") && (evt.value == "open")) { // If a person opens an unlocked door...
